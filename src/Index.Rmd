...
nara2016 <- terra::rast(sprintf("%s/data/2016/LG2016_finaal_ori.tif",here()))
nara2013 <- terra::rast(sprintf("%s/data/2013/LG2013_finaal_ori.tif",here()))
nara16 <- cleanmapdata(nara2016, points_id, tbltrans)
nara13 <- cleanmapdata(nara2013, points_id, tbltrans)
...
bosw16 <- cleanmapdata(bosw2016, points_id, tbltrans)
bosw13 <- cleanmapdata(bosw2013, points_id, tbltrans)
...
bak16 <- cleanmapdata(bak2016, points_id, tbltrans)
bak13 <- cleanmapdata(bak2013, points_id, tbltrans)
...
gras16 <- cleanmapdata(gras2016, points_id, tbltrans)
gras13 <- cleanmapdata(gras2013, points_id, tbltrans)
...
all16 <- cleanmapdata(all2016, points_id, tbltrans)
all13 <- cleanmapdata(all2013, points_id, tbltrans)
```
...
## Results for the original map
```{r results='asis', warning = FALSE, message = FALSE}
title <- "the original land use maps"
mapdata1 <- nara13$valid_eng
refdata1 <- as.factor(points_id$lu13oord_eng)
mapdata2 <- nara16$valid_eng
refdata2 <- as.factor(points_id$lu16oord_eng)
maparea <- Cleanchangeareadata(
  file = "data/2013_2016/LG2013_LG2016_finaal_ori.csv", 
  tbltrans = tbltrans)
options(knitr.duplicate.label = "allow")
```
```{r callchild1, child = sprintf('%s/src/template/child.Rmd',here()), fig.path='figure/childnara-'}

```
## Results for the updated map with boswijzer
```{r results='asis', warning = FALSE, message = FALSE}
title <- "the original land use maps, updated with the boswijzer"
mapdata1 <- bosw13$valid_eng
refdata1 <- as.factor(points_id$lu13oord_eng)
mapdata2 <- bosw16$valid_eng
refdata2 <- as.factor(points_id$lu16oord_eng)
maparea <- Cleanchangeareadata(
  file = "data/2013_2016/LG2013_LG2016_boswijzer_update.csv", 
  tbltrans = tbltrans)
options(knitr.duplicate.label = "allow")
```
```{r callchild2, child = sprintf('%s/src/template/child.Rmd',here()), fig.path='figure/childbos-'}

```
## Results for the updated map with BAK
```{r results='asis', warning = FALSE, message = FALSE}
title <- "the original land use maps, updated with BAK"
mapdata1 <- bak13$valid_eng
refdata1 <- as.factor(points_id$lu13oord_eng)
mapdata2 <- bak16$valid_eng
refdata2 <- as.factor(points_id$lu16oord_eng)
maparea <- Cleanchangeareadata(
  file = "data/2013_2016/LG2013_LG2016_BAK_update.csv", 
  tbltrans = tbltrans)
options(knitr.duplicate.label = "allow")
```

```{r callchild3, child = sprintf('%s/src/template/child.Rmd',here()), fig.path='figure/childbak-'}

```

## Results for the updated map with improved grassland

```{r results='asis', warning = FALSE, message = FALSE}
title <- "the original land use maps, updated with improved grassland"
mapdata1 <- gras13$valid_eng
refdata1 <- as.factor(points_id$lu13oord_eng)
mapdata2 <- gras16$valid_eng
refdata2 <- as.factor(points_id$lu16oord_eng)
maparea <- Cleanchangeareadata(
  file = "data/2013_2016/LG2013_LG2016_grasland_update.csv", 
  tbltrans = tbltrans)
options(knitr.duplicate.label = "allow")
```

```{r callchild4, child = sprintf('%s/src/template/child.Rmd',here()), fig.path='figure/childgras-'}

```

## Results for the updated map, improved with boswijzer, BAK, and grasslands

```{r results='asis', fig.path='figure/childall-', warning = FALSE, message = FALSE}
title <- "the original land use maps, updated with BAK"
mapdata1 <- all13$valid_eng
refdata1 <- as.factor(points_id$lu13oord_eng)
mapdata2 <- all16$valid_eng
refdata2 <- as.factor(points_id$lu16oord_eng)
maparea <- Cleanchangeareadata(
  file = "data/2013_2016/LG2013_LG2016_finaal_update.csv", 
  tbltrans = tbltrans)
options(knitr.duplicate.label = "allow")
```

```{r callchild5, child = sprintf('%s/src/template/child.Rmd',here()), fig.path='figure/childall-'}
